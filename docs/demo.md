...
本页记录 2026-08-28 的真实本地操作：第 1–5 节在 Workflow Studio 中运行三 Agent 流程，第 6 节使用随后部署的独立 Docker 执行器验证修复。
...
以下两张截图来自另一次验收环境：`127.0.0.1:8082`，独立 Lima Docker VM、
PostgreSQL + Redis、已认证管理员和私有 socket Proof 执行器，与上表的早期规则演示不同。
Web 本身没有 Docker 客户端或控制 socket；使用方式见
[本地部署说明](local-docker.md)。

在「修复验证」中选择演示样例，直接编辑修复前后的文件与测试命令，不用填写 JSON。
可以新增/删除文件，或用勾选框表示文件只存在于一个版本。

![真实修复验证页面：前后代码及复现和回归命令](images/proof-editor.png)

本次样例的 `clamp(-1)` 在修复前返回负数，断言失败；补丁将下限限制为 0，
相同断言通过，再检查 0、5、20 的回归输入。三个阶段实际在受限容器中执行，
得到 L4；这里的第一个「测试失败」是预期的复现证据，不是系统故障。

![实际 L4 证据：修复前失败、修复后通过、回归通过](images/proof-result.png)

还验证了文件增删、修改输入清除旧结果、退出登录清除文件，以及停止执行器后显示
「无法判断」、恢复后重新执行成功。没有回退到宿主机执行，也没有向 GitHub 写入。
审计记录保留与请求及镜像绑定的摘要，使用者页面只展示可读的证据。

L4 只覆盖提供的文件和测试命令，不等于所有潜在缺陷都被修好；这是手动 Proof，
不是自动生成补丁，也不是新的 Studio 流程节点。完整范围见
[隔离验收记录](integration-testing.md#private-proof-executor-and-console-acceptance)。
...
已具备可视化创建/组合 Agent、版本固定、真实试运行、可读报告和可恢复交接；
新增独立 Docker 的手动修复验证页面。仓库还记录了完整回归、打包和多副本受控压测的条件与结果。
执行器进程强杀、沙箱自动回收及恢复后 L4 已通过 [实机验收](integration-testing.md#sandbox-owner-loss-acceptance)。

仍待完成：真实模型和 GitHub 操作联调、VM/daemon 断电或暂停后的容器协调、代表性负载/SLO，
以及独立标注的真实 PR 评测。受控样本的端到端修复率为 35%，未达 60% 门槛；
不能用演示成功、测试覆盖率或 CI 绿灯替代这些要求。
