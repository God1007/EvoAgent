...
| **灾备证据** | SQLite 在线快照与 PostgreSQL MVCC `pg_dump` 均执行隔离恢复，校验迁移、Schema、逐表内容指纹、应用读写及 RPO/RTO |
